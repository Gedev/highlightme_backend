...
def index(request):
    ...
    try:
        ...
        grouped_fights = analyze_report(fights)  # Initially pass empty boss_names

        # Fetch events data and create highlights for each difficulty
        all_highlights = []
        for difficulty_name, difficulty_fights in grouped_fights.items():
            try:
                if settings.DEBUG:
                    ...
                    # Extraire la difficulté de manière sûre à partir de difficulty_fights
                    difficulty = next(iter(difficulty_fights), {}).get('difficulty', None)

                    # Vérifier si la difficulté a été correctement extraite
                    if difficulty is None:
                        logger.error(f"Could not determine difficulty for fights: {difficulty_fights}")
                        return JsonResponse({'error': 'Difficulty not found for fights'}, status=500)
